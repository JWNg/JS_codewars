...
ALGORITHMSFUNDAMENTALSCRYPTOGRAPHYSECURITYSTRINGSARRAYS

const keyboard = ["qwertyuiop", "asdfghjkl", "zxcvbnm,.", "ZXCVBNM<>"]

function encrypt(text, key) {
    return text.split('').map((letter)=>{return encryptor(letter, key, false)}).join('')
}

function decrypt(text, key) {
    return text.split('').map((letter)=>{return encryptor(letter, key, true)}).join('')
}


function encryptor(letter, key, decrypt) {
    var splitKey = (key === 0 ? ['0', '0', '0']:(key+'').split(''));
    if(keyboard[0].includes(letter)) {
        return movePosition(letter, keyboard[0].split(''), parseInt(splitKey[0]), decrypt)
    } else if (keyboard[1].includes(letter)) {
        return movePosition(letter, keyboard[1].split(''), parseInt(splitKey[1]), decrypt)
    } else if (keyboard[2].includes(letter)) {
        return movePosition(letter, keyboard[2].split(''), parseInt(splitKey[2]), decrypt)
    } else if (keyboard[0].toUpperCase().includes(letter)) {
        return movePosition(letter, keyboard[0].toUpperCase().split(''), parseInt(splitKey[0]), decrypt)
    } else if (keyboard[1].toUpperCase().includes(letter)) {
        return movePosition(letter, keyboard[1].toUpperCase().split(''), parseInt(splitKey[1]), decrypt)
    } else if (keyboard[3].includes(letter)) {
        return movePosition(letter, keyboard[3].toUpperCase().split(''), parseInt(splitKey[2]), decrypt)
    } else {
        return letter
    }
}
...
//line is an array
function movePosition(letter, line, positions, decrypt){
    let index = decrypt ? line.reverse().indexOf(letter) : line.indexOf(letter);
    let newIndex = (index+positions) % line.length;
