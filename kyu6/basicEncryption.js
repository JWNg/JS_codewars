...
Good luck.
PUZZLESGAMESENCRYPTIONALGORITHMSCRYPTOGRAPHYSECURITYDECRYPTIONMATHEMATICSNUMBERSSTRINGS


function encrypt(text, rule) {
  const changeLetter = (letter, rule) => {
    return String.fromCharCode((letter.charCodeAt()+rule)%256)
  }
  return text.split('').map(letter => changeLetter(letter, rule)).join('')
};
