...
ALGORITHMSBASIC LANGUAGE FEATURESFUNDAMENTALSARRAYSCONTROL FLOW



function splitAllEvenNumbers(xs, w) {
  return xs.reduce((a, x) => (a.push.apply(a, split(x, w)), a), []);
}

function split(n, w) {
  if ((n & 1) != 0) return [n];
  switch (w) {
    case 0: return (((n >>= 1) & 1) == 0) ? [n - 1, n + 1] : [n, n];
    case 1: return [1, n - 1];
    case 2:
    {
      let d = n, q = 1; do d >>= 1, q <<= 1; while ((d & 1) == 0);
      return Array.from({length: q}, _ => d);
    }
    case 3: return Array.from({length: n}, _ => 1);
  }
}
