...
function mean(list){
  let num = list.filter((item) => {return parseInt(item) || item === '0'})
  let joinedLetters = list.filter((item) => {return !parseInt(item) && item !== '0'}).join('')
  let average = parseFloat((num.reduce((sum, num)=>{return parseInt(sum)+parseInt(num)}) / (num.length)).toFixed(1));
  
  return [average, joinedLetters]
}
