...
function numberJoy(n) {
  var split = (n+"").split('').map(num => parseInt(num));
  var sum = split.reduce((sum, next) => {return sum + next});
  var reverseSum = parseInt((''+sum).split('').map(num => parseFloat(num)).reverse().join(''));
